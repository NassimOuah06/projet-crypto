...
class PatientsPage(QWidget):
    BASE_API_URL = "http://127.0.0.1:8000/api/admin/patients/"
    ...
    def add_patient(self):
        dialog = QDialog(self)
        dialog.setWindowTitle("Ajouter un Nouveau Patient")
        dialog.setFixedSize(500, 450)  # Taille légèrement augmentée
        ...
        # Titre stylisé
        title = QLabel("AJOUTER UN PATIENT")
        title.setStyleSheet("""
            QLabel {
                font-size: 18px;
                font-weight: bold;
        ...
        fields = [
            ...
            ("Statut de Traitement:", QComboBox())
        ]
        fields[-1][1].addItems(["non_traité", "traité"])
        ...
        submit_btn = QPushButton("Ajouter Patient")
        ...
        def send_data():
            patient_data = {
                ...
                "etat_traitement": fields[5][1].currentText()
            }
            ...
            try:
                ...
                if response.status_code == 201:
                    ...
                    error = response.json().get('detail', 'Failed to add patient')
                    QMessageBox.critical(dialog, "Error", error)
            except requests.exceptions.RequestException as e:
                QMessageBox.critical(dialog, "Error", f"Connection error: {str(e)}")
